...
### Run flags
num_ions=15
scan_type = 'calibrate_J_Hamil'   #### 'calibrate_J'  |  'motional_freq'   |'imbl'| 'N_cut_seg'  | 'calibrate_J_Hamil'  | 'phi_SK1' |  'LS'
...
if(scan_type=='motional_freq'):
    ...
    amp_ind_scale = 0.22#0.2 ### we measured J of 3.8765 for amplitude of 0.2 (including the Tpi relative calibration)

amp_imbal_ratio_b2r_vec = [1.019]
LS_scan_vec = [0]
N_cut_seg_vec =[0] #### how many segments to cut from the end of the main pulse

# LS_vec_calib =0*np.array([0,-702.9833453013276, -369.24042962936767, -176.6579521955088, 0, 0,0, 0, 112.06664386185497, 169.34629255725625, 272.8985047400167, 326.74836038805495, 294.587680659517, 209.86416245725263,0])#0+np.zeros(num_ions) #np.array([0,-745.4462324776171, -396.39619246543265, 0, 0, 0, 0, 16.948460438199866, 113.10989385584325, 70.74618532965478, 227.84928209623544, 258.7078158209746, 210.94526305822177, 144.6745026726266,0])
LS_vec_calib = 0*np.array([0,-946.2314221935225, -496.04642861282326, -270.6149181366428, -137.82605147481092, -108.59294689317466, 0, 0,0, 98.88819604464994, 168.83762434024035, 220.37080609382866, 143.75073348208622, 0,0])#0+np.zeros(num_ions) #np.array([0,-745.4462324776171, -396.39619246543265, 0, 0, 0, 0, 16.948460438199866, 113.10989385584325, 70.74618532965478, 227.84928209623544, 258.7078158209746, 210.94526305822177, 144.6745026726266,0])
LS_vec_calib[1] = -760*0.8*1.08
...
if(scan_type == 'calibrate_J'):
    ...
    Npts = 31
    detuning_vec = 0*detuning_vec[0]+np.linspace(-7e3,7e3,Npts)
    ...
    Npts = 19
    amp_imbal_ratio_b2r_vec = 1.022+np.linspace(-0.005,0.005,Npts)
