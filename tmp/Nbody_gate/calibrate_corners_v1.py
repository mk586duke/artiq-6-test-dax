...
def corner(chan_glob_vec,f_carrier_glob,chan_ind_vec,f_carrier_ind_vec,chan_ind_pad_vec,freq_ind_pad,
    ...
    ### load splines
    this_dir = pathlib.Path(__file__).parent.resolve()
    json_file = this_dir / "splines_squeeze_compact__mode3_pulsedur034.json"
    data_splines = json.loads(json_file.read_text())
...
T_pi_vec_microsec = np.array([3.542,3.089,3.597])#this should be
...
### Main Schedule
schedules = []
rel_phase_vec = [0]#np.linspace(-0.1,0.1,10)#np.array([0*np.pi/2])#np.array([np.pi/2]) # np.array([0])#

amp_imbal_ratio_b2r_disp_vec = np.array([1.015])#
Amp_norm_ind_disp_vec =  np.array([0.962])#np.linspace(0.952,0.973,9)
w_k = 2.8245e6

amp_imbal_ratio_b2r_squz_vec = [1.0182]#np.linspace(1.013,1.027,25)#]# at 0.7 ind amp it is 1.018
Amp_norm_ind_squz_vec =np.array([0.2])#np.linspace(0,1,11)#
w_k_squz_vec = np.linspace(1.9e6,3e6,81)#[2.824e6]#np.linspace(2.823e6,2.826e6,21)#[2.825e6]###np.array([2.839e6])##np.linspace(2.836e6,2.843e6,11)##np.linspace(2.80e6,2.86e6,21)#np.linspace(2.825e6,2.83e6,21)#

tau_wait = int(1e1)#np.linspace(1e1,1e3,11)#
light_shift = 0
        ...
        for phi_scan in rel_phase_vec:
            for w_k_squz in w_k_squz_vec:
                with qp.build(backend=qp_backend) as out_sched:
                    ...
                    # sequential_sk1(applied_gates=[False, True, False], pi_times=T_pi_vec_microsec,
                    #                 thetas=np.array([0,np.pi/2,0]), phis=np.array([0, 0, 0]))
                    ...
                    for nn in range(N_stg):
                        ...
                        phase_correction2 = phase_correction2+ 2*np.pi*qp.samples_to_seconds(duration_squz)*(freq_global_vec_SB0[0]-freq_global_vec_SB2[0])
                        # if(nn==1):
                        #     sequential_sk1(applied_gates=[False, True, False], pi_times=T_pi_vec_microsec,
                        #                     thetas=np.array([0,np.pi,0]), phis=np.array([0, 0, 0])+0+phase_correction2)

                    # sequential_sk1(applied_gates=[False, True, False], pi_times=T_pi_vec_microsec,
                    #                 thetas=np.array([0,np.pi,0]), phis=np.array([0, 0, 0])+np.pi+phase_correction2)

                    # sequential_sk1(applied_gates=[False, True, False], pi_times=T_pi_vec_microsec,
                    #                 thetas=np.array([0,np.pi/2,0]), phis=np.array([0, 0, 0])+np.pi+phase_correction2)
