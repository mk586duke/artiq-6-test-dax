...
seq_type1 = 'Displacement' #### 'Displacement'  |  'Squeezing'  |  'Both'  |  'Sqz_Clos'  |
seq_type2 = 'Closure' #### 'Closure'  |  'Gate'  |
...
T_pi_vec_microsec =  np.array([3.97,3.542,3.985])##np.array([4.411,3.823,4.429])#this should be
...
amp_imbal_ratio_b2r_disp_vec = np.array([1.01])#np.array([1.01])#
amp_imbal_ratio_b2r_squz_vec = [1.0125]# [1.0178]# at 0.7 ind amp it is 1.018
...
if(scan_type=='Freq'):
    if((seq_type1=='Displacement')or(seq_type1=='Both') or(seq_type1=='Sqz_Clos')):
        w_k_disp_vec =  (w_k_disp_vec+freq_offset)+np.linspace(-4e2,4e2,15)#np.linspace(2.812e6,2.815e6,17)#np.linspace(2.79e6,2.83e6,15)#   np.linspace(2.8094e6,2.8098e6,15)
    elif((seq_type1=='Squeezing')):
    ...
    if((seq_type1=='Displacement') or(seq_type1=='Sqz_Clos')):
        ...
        amp_imbal_ratio_b2r_disp_vec = np.linspace(1.005,1.015,11)
    elif((seq_type1=='Squeezing')or (seq_type1=='Both')):
        amp_imbal_ratio_b2r_squz_vec = np.linspace(1.01,1.02,21)#]
    ...
    # phi_Parity_vec = np.linspace(-0.15,2*np.pi/3,7)
    phi_Parity_vec = np.linspace(-0.1,0.1,21)
elif(scan_type=='Nrep'):
    N_gate_rep_vec = np.arange(4)
elif(scan_type=='LS'):
    LS_vec = 1.3+np.linspace(-2e2,2.5e2,21)
                                        ...
                                        for Phi12 in Phi12_vec:
                                            for phi_Parity in phi_Parity_vec:
                                                with qp.build(backend=qp_backend) as out_sched:
                                                    ...
                                                    ### 2nd sidebands
                                                    spin_phase_squz = 0.03+ np.array([0,0,0,0])+np.pi/2+0.05
                                                            ...
                                                            if(is_Echo):
                                                                if(np.mod(nn,2)==1):
                                                                    ...
                                                                    T_echo = sequential_sk1(applied_gates=spin_echo, pi_times=T_pi_vec_microsec,
                                                                                        thetas=np.array([np.pi,np.pi,np.pi]), phis=np.array([np.pi/2+0.03, 0.03+np.pi/2,np.pi/2+0.03])+0.04+(np.pi*phase_echo)+phase_correction_echo+phiSK1_vec_bare)
