...
Amp_norm_global_cm_disp = 0.35
Amp_norm_global_cm_squz = 0.45
amp_imbal_ratio_b2r_disp = 1.01
...
T_pi_vec_microsec = np.array([1.07,0.93,1.06])

N_stg =4
N_stg_vec =[0,0,0,0]
ion_idx_disp_vec  = [Ions_displacement_ind[0],Ions_displacement_ind[0],Ions_displacement_ind[1],Ions_displacement_ind[1]] #[Ions_displacement_ind[0],Ions_displacement_ind[1],Ions_displacement_ind[0],Ions_displacement_ind[1]] ### the ion that is displaced as a function of the stage number
...
### Main Schedule
schedules = []
light_shift_vec = np.array([0])#np.linspace(-2e3,2e3,21)# #
Amp_norm_ind_disp_vec =  np.array([0.99]) #np.linspace(0,0.99,21)
rel_phase_vec = np.array([np.pi/2]) # np.linspace(0,np.pi,21)#np.array([0])#
w_k_vec = np.linspace(2.8265e6,2.83e6,11)#np.array([2.827e6])#np.linspace(2.80e6,2.86e6,21)#np.linspace(2.825e6,2.83e6,21)#
tau_wait = int(1e4)
        ...
        for phi_scan in rel_phase_vec:
            for w_k in w_k_vec:
                with qp.build(backend=qp_backend) as out_sched:
                    ...
                    spin_phase_disp = [0,0.0,0,0]
                    motion_phase_offset_disp = [0*np.pi,np.pi,0.5*np.pi,1.5*np.pi] ### each stage the phase is increased by pi/2 through the optimizer solution. This should be a small offset / correction
...
# x_values = light_shift_vec
# x_label = "light shift [kHz]"
# x_values = rel_phase_vec
# x_label = "rred blue spin phase"
