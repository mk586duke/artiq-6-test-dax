...
Amp_norm_ind_disp = 0.5
Amp_norm_ind_squz = 0.5
Amp_norm_global_cm_disp = 0.25
Amp_norm_global_cm_squz = 0.45
amp_imbal_ratio_b2r_disp = 1.0
...
T_pi_vec_microsec = np.array([1.11,0.94,1.08])

N_stg = 1
N_stg_vec =[0,0]
ion_idx_disp_vec  = [Ions_displacement_ind[1],Ions_displacement_ind[1]] #[Ions_displacement_ind[0],Ions_displacement_ind[1],Ions_displacement_ind[0],Ions_displacement_ind[1]] ### the ion that is displaced as a function of the stage number
...
### Main Schedule
schedules = []
rel_phase_vec = np.linspace(0,np.pi,19)#np.array([0])
w_k_vec = np.array([2.828e6])#-np.linspace(2.80e6,2.86e6,21)#
for phi_scan in rel_phase_vec:
    for w_k in w_k_vec:
        with qp.build(backend=qp_backend) as out_sched:
            ...
            motion_phase_offset_disp = [0.5*np.pi,1.5*np.pi] ### each stage the phase is increased by pi/2 through the optimizer solution. This should be a small offset / correction
            freq_carrier_glob = qp_backend.properties().rf_calibration.frequencies.global_carrier_frequency.value
            freq_global_vec_SB0 = np.array([freq_carrier_glob,freq_carrier_glob]) # red and blue tones' frequencies in Hz for first sideband
            ...
            sequential_Rabi(T_rotation_rel  = np.array([0.0,0.0,0.5]),spin_phase_Rabi = np.zeros(3))
            ...
            #     phase_rad=0,output_enable=False,sync=False, _name="rabi_global_delay"), chan_glob_tone_red)

            # qp.play(ToneDataPulse(dT_delay, frequency_hz = freq_global_vec_SB0[1], amplitude = 0.0,
            #     phase_rad=0,output_enable=False,sync=False, _name="rabi_global_zero_delay"), chan_glob_tone_blue)

            # qp.play(ToneDataPulse(dT_delay, frequency_hz = freq_ind_vec[1], amplitude = 0.0,
            #     phase_rad=0,output_enable=False,sync=False, _name="rabi_individual_delay"), chan_ind_vec[1])
            # phase_correction = 2*np.pi*(freq_global_vec_SB0[0]-freq_global_vec_SB1[0])*qp.samples_to_seconds(dT_delay)



            # breakpoint()
            sequential_Rabi(T_rotation_rel  = np.array([0.0,0.0,0.5]),spin_phase_Rabi =  np.array([np.pi,np.pi,np.pi]) + phase_correction)
