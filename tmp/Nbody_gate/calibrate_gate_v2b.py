...
scan_type = 'Freq' #### 'Freq'  |  'ImbGlob'   |  'IndAmp'  | 'Tdelay'  |  'phiSK1'  | 'Phi12'  | 'Parity' | ' LS'  | 'Nrep'
is_Ramsey = False
is_Echo = True
is_initial_pi_pulse = False
N_stg = 4
spin_flip =  [False,True,True]
...
T_pi_vec_microsec =  np.array([3.684,3.254,3.705])##np.array([4.411,3.823,4.429])#this should be
...
amp_imbal_ratio_b2r_squz_vec = [1.0182]# [1.0178]# at 0.7 ind amp it is 1.018
...
if(scan_type=='Freq'):
    if((seq_type1=='Displacement')or(seq_type1=='Both') or(seq_type1=='Sqz_Clos')):
        w_k_disp_vec =  (w_k_disp_vec+freq_offset)+np.linspace(-1.5e2,1.5e2,5)#np.linspace(2.812e6,2.815e6,17)#np.linspace(2.79e6,2.83e6,15)#   np.linspace(2.8094e6,2.8098e6,15)
    elif((seq_type1=='Squeezing')):
    ...
    if((seq_type1=='Displacement') or(seq_type1=='Sqz_Clos')):
        ...
        amp_imbal_ratio_b2r_squz_vec = np.linspace(1.007,1.02,27)#]
    ...
    if((seq_type1=='Displacement')or(seq_type1=='Both') or(seq_type1=='Sqz_Clos')):
        ...
        Amp_norm_ind_disp_vec = np.array([1.0, 1.05, 1.1, 1.15,1.2, 1.25, 1.3, 1.35, 1.4, 0.7, 0.78, 0.85, 0.93 , 0.4, 0.5, 0.6,  0.01, 0.1, 0.25])# 0.6+np.linspace(0.01,0.5,11) #np.linspace(0.6,0.85,15)
    elif(seq_type1=='Squeezing'):
        Amp_norm_ind_squz_vec = np.linspace(0.67,0.57,11)#

elif(scan_type=='phiSK1'):
    # phiSK1_vec = np.linspace(-0.3,0.3,11)
    phiSK1_vec = np.linspace(-0.1,0.16,17)
    ...
    # phi_Parity_vec = np.linspace(0,2*np.pi,13)
    phi_Parity_vec = np.linspace(-0,0.2,13)
elif(scan_type=='Nrep'):
    N_gate_rep_vec = [0,1,2,3,4]#np.arange(6)
elif(scan_type=='LS'):
    LS_vec = 0+np.linspace(0e2,7.5e2,13)
                                        ...
                                        for Phi12 in Phi12_vec:
                                            for phi_Parity in phi_Parity_vec:
                                                with qp.build(backend=qp_backend) as out_sched:
                                                    ...
                                                    ### 2nd sidebands
                                                    spin_phase_squz = 0.03+ np.array([0,0,0,0])+np.pi/2+0.04
                                                    ...
                                                    if(is_Ramsey):
                                                        T_ramsey1 = sequential_sk1(applied_gates=spin_ramsey, pi_times=T_pi_vec_microsec,
                                                                        thetas=np.array([np.pi/2,np.pi/2,np.pi/2]), phis=np.array([0.02,0.02, 0.02])+phiSK1_vec_bare+phiSK1)
                                                            ...
                                                            if(is_Echo):
                                                                if(np.mod(nn,2)==1):
                                                                    ...
                                                                    T_echo = sequential_sk1(applied_gates=spin_echo, pi_times=T_pi_vec_microsec,
                                                                                        thetas=np.array([np.pi,np.pi,np.pi]), phis=np.array([np.pi/2+0.02,0.03+np.pi+np.pi/2,np.pi/2+0.02])+0.04+(np.pi*phase_echo)+phase_correction_echo+phiSK1_vec_bare)
                                                    ...
                                                    if(is_Ramsey):
                                                        T_ramsey2 = sequential_sk1(applied_gates=spin_ramsey, pi_times=T_pi_vec_microsec,
                                                                            thetas=np.array([np.pi/2,np.pi/2,np.pi/2]), phis=np.array([0.02,0.02+-0.08, 0.02])+np.pi+phase_correction_tot+phiSK1_vec_bare+phiSK1)
