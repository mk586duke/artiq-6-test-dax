...
scan_type = 'Freq' #### 'Freq'  |  'ImbGlob'   |  'IndAmp'  | 'Tdelay'  |  'phiSK1'  | 'Phi12'  | 'Parity' | ' LS'
is_Ramsey = False
is_Echo = True
...
T_pi_vec_microsec =  np.array([3.768,3.349,3.782])##np.array([4.411,3.823,4.429])#this should be
...
phiSK1_vec_bare = np.array([0,0.18,0])#np.array([0.0,0.18,0.0])
phiSK1_vec = np.array([np.pi]);#
Phi12_vec = [1.7]
phi_Parity_vec = [-0.1]
LS_vec = [1.3e3]
LS_carr = 1.3e3
...
if(scan_type=='Freq'):
    if((seq_type1=='Displacement')or(seq_type1=='Both') or(seq_type1=='Sqz_Clos')):
        w_k_disp_vec =  w_k_disp_vec+freq_offset+np.linspace(-1e2,1e2,11)#np.linspace(2.812e6,2.815e6,17)#np.linspace(2.79e6,2.83e6,15)#   np.linspace(2.8094e6,2.8098e6,15)
    elif((seq_type1=='Squeezing')):
    ...
    phiSK1_vec = np.linspace(-0.25,0.25,15)
    ...
    Phi12_vec = Phi12_vec[0]+np.linspace(-0.3,0.3,9)
    # Phi12_vec = Phi12_vec[0]+np.array([0,0,np.pi,np.pi,0,np.pi,0,np.pi])
elif(scan_type=='Parity'):
    phi_Parity_vec = np.linspace(-0.15,0.15,13)
elif(scan_type=='Nrep'):
    N_gate_rep_vec = np.arange(4)
elif(scan_type=='LS'):
    LS_vec = np.linspace(-1e3,1e3,17)


N_run_freq = 5
N_run_var = 8

w_k_eff_vec_tmp = np.copy(w_k_disp_vec[0])+np.matlib.repmat(np.array([0,-50,30,60,90]),N_run_var,1)
w_k_disp_vec = w_k_eff_vec_tmp.flatten('F')
w_k_squz_vec = [0]
print(w_k_disp_vec)
...
is_parity = False
if(is_amp):
    Amp_norm_ind_disp_tmp = np.matlib.repmat(np.hstack((artiq_data.get("data.Nbody.amp_disp"),np.linspace(0.8,1.4,N_run_var-1))),N_run_freq,1)
    Amp_norm_ind_disp_vec2 = Amp_norm_ind_disp_tmp.flatten('C')
    print(Amp_norm_ind_disp_vec)
if(is_parity):
    is_parity_vec= np.matlib.repmat(np.hstack((1,np.ones(N_run_var-1))),N_run_freq,1)
    phi_Parity_vec2= np.matlib.repmat(np.hstack((0,np.linspace(0,2*np.pi/6,N_run_var-1))),N_run_freq,1)

counter_scan = 0
for N_gate_rep in N_gate_rep_vec:
    for mm in range(N_run_freq*N_run_var):
        for ls in LS_vec:
                                        ...
                                        for Phi12 in Phi12_vec:
                                            for phi_Parity in phi_Parity_vec:
                                                with qp.build(backend=qp_backend) as out_sched:
                                                    ...
                                                    w_k_squz = w_k
                                                    if(is_amp):
                                                        Amp_norm_ind_disp = Amp_norm_ind_disp_vec2[mm]
                                                    if(is_parity):
                                                        phi_Parity= phi_Parity_vec2[mm]
                                                    ...
                                                    # T_ramsey2 = sequential_sk1(applied_gates=spin_ramsey, pi_times=T_pi_vec_microsec,
                                                    #                     thetas=np.array([0,phi_Parity,0]), phis=np.array([0.05,0.05+-0.08, 0.05])+phase_correction_tot+phiSK1_vec_bare+phiSK1)
