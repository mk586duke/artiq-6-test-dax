...
N_stg = 4
N_stg_vec =[0,0,0,0]
ion_idx_disp_vec  = [Ions_displacement_ind[0],Ions_displacement_ind[1],Ions_displacement_ind[0],Ions_displacement_ind[1]] #[Ions_displacement_ind[0],Ions_displacement_ind[1],Ions_displacement_ind[0],Ions_displacement_ind[1]] ### the ion that is displaced as a function of the stage number
...
### Main Schedule
schedules = []
Amp_norm_ind_disp_vec =np.linspace(0,0.99,21)
rel_phase_vec = np.array([np.pi/2]) # np.linspace(0,np.pi,21)#np.array([0])#
w_k_vec = np.array([2.827e6])#-np.linspace(2.80e6,2.86e6,21)#np.linspace(2.825e6,2.83e6,21)#
    ...
    for phi_scan in rel_phase_vec:
        for w_k in w_k_vec:
            with qp.build(backend=qp_backend) as out_sched:
                ...
                motion_phase_offset_disp = [0*np.pi,0.5*np.pi,1*np.pi,1.5*np.pi] ### each stage the phase is increased by pi/2 through the optimizer solution. This should be a small offset / correction
