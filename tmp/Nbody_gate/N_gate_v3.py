...
def edge(chan_glob_vec,f_carrier_glob,chan_ind,f_carrier_ind,chan_ind_pad_vec,freq_ind_pad,
    ...
    ### load splines
    f = open("splines_displace_xpxp_mode3_pulsedur026_waitdur024.json")
    data_splines = json.load(f)
    f.close()
...
def corner(chan_glob_vec,f_carrier_glob,chan_ind_vec,f_carrier_ind_vec,chan_ind_pad_vec,freq_ind_pad,
    ...
    ### load splines
    f = open("splines_squeeze_compact__mode3_pulsedur024.json")
    data_splines = json.load(f)
    f.close()
...
schedules = []
qp_backend = qbe.get_default_qiskit_backend(master_ip, 1, with_2q_gate_solutions=False)
channel_map = qbe.get_default_rfsoc_map()
three_ion_config = QuickConfig(3,channel_map,[-1:1,0:3,1:5])


w_k = 2.822e6 ### mode frequency from carrier
...
# freq_ind_vec = np.array([15e6,24e6,18e6,19e6]) #frequencies in Hz
Ions_displacement_ind = np.array([0,2],int)
Ions_squeezing_ind = np.array([1],int)
Amp_norm_ind_disp = 0.9
...
N_stg = 2
ion_idx_disp_vec  = [Ions_displacement_ind[0],Ions_displacement_ind[1],Ions_displacement_ind[0],Ions_displacement_ind[1]] ### the ion that is displaced as a function of the stage number
spin_phase_disp = [0,0,0,0]
motion_phase_offset_disp = [0,0,0,0] ### each stage the phase is increased by pi/2 through the optimizer solution. This should be a small offset / correction
...
with qp.build(backend=qp_backend) as out_sched:
    ...
    chan_ind_vec = []
    for ion_idx in range(-int((-1+freq_ind_vec.shape[0])/2),int((-1+freq_ind_vec.shape[0])/2)):
    ...
    for nn in range(N_stg):
        ...
        [duration_disp] = edge(chan_glob_vec=chan_glob_vec,f_carrier_glob = freq_global_vec_SB1, chan_ind=chan_ind_vec[ion_idx_stg],
                                    ...
                                    phi_motion_offset = motion_phase_offset_disp[nn],n_edge = nn) ### displacement of a single ion
        ## Squeezing stage
...
# upload_channel_sequence(compiled_schedule, num_repeats=100, rfsoc_channel_map=channel_map, streaming=stream_mode)
