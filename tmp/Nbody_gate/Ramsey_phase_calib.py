...
T_pi_vec_microsec = np.array([1.63,1.41,1.6])
...
for w_k in w_k_vec:
    with qp.build(backend=qp_backend) as out_sched:
        ...
        freq_global_vec_SB0 = np.array([f_carrier_glob,f_carrier_glob]) # red and blue tones' frequencies in Hz for first sideband
        ...
        three_Rabi(T_rotation_rel  = np.array([0.5,0.0,0.0]))
        ...
        # duration_disp = 0
        phase_correction = 2*np.pi*qp.samples_to_seconds(duration_disp)*N_stg*(freq_global_vec_SB1[1]-f_carrier_glob)
        three_Rabi(T_rotation_rel  = np.array([0.5,0.0,0.0]),spin_phase_Rabi =  np.array([np.pi,np.pi,np.pi])+phase_correction)
