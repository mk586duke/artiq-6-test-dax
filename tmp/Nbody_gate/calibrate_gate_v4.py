...
def edge(chan_glob_vec,f_carrier_glob,chan_ind,f_carrier_ind,chan_ind_pad_vec,freq_ind_pad,
    ...
    freq_red = f_carrier_glob[0]+LS
    freq_blue = f_carrier_glob[1]+LS
...
scan_type = 'Freq' #### 'Freq'  |  'ImbGlob'   |  'IndAmp'  | 'Tdelay'  |  'phiSK1'  | 'Phi12'  | 'Parity' |
is_Ramsey = False
is_Echo = True
...
T_pi_vec_microsec =  np.array([4.101,3.486,4.061])##np.array([4.411,3.823,4.429])#this should be
...
if(is_get_dataset_vals):
    ...
    Amp_norm_ind_disp_vec = [artiq_data.get("data.Nbody.amp_disp")]
    Amp_norm_ind_squz_vec = [artiq_data.get("data.Nbody.amp_squeez")]
else:
...
amp_imbal_ratio_b2r_disp_vec = np.array([1.009])#np.array([1.01])#
amp_imbal_ratio_b2r_squz_vec = [1.0178]# at 0.7 ind amp it is 1.018
w_k_squz_vec = w_k_disp_vec#[2.811e6]#

phiSK1_vec_bare = np.array([0.0,0.18,0.0])#np.array([0.0,0.18,0.0])
phiSK1_vec = np.array([0*np.pi/2]);#
Phi12_vec = [1.7]
phi_Parity_vec = [-0.1]
N_gate_rep_vec = [2]
...
if(seq_type1=='Displacement'):
    tau_wait_vec= [int(10000)]# [int(14000)]given in machine units
elif(seq_type1=='Squeezing'):
    tau_wait_vec= [int(10600)]# given in machine units
...
if(scan_type=='Freq'):
    if((seq_type1=='Displacement')or(seq_type1=='Both') or(seq_type1=='Sqz_Clos')):
        w_k_disp_vec =  w_k_disp_vec+freq_offset+np.linspace(-1e2,1e2,7)#np.linspace(2.812e6,2.815e6,17)#np.linspace(2.79e6,2.83e6,15)#   np.linspace(2.8094e6,2.8098e6,15)
    elif((seq_type1=='Squeezing')):
        w_k_squz_vec =  w_k_squz_vec+np.linspace(-3e2,3e2,13)#np.linspace(2.79e6,2.83e6,15)np.linspace(2.808e6,2.813e6,21)#
    ...
    if((seq_type1=='Displacement') or(seq_type1=='Sqz_Clos')or (seq_type1=='Both')):
        ...
        amp_imbal_ratio_b2r_disp_vec = np.linspace(1.0,1.02,9)
    elif((seq_type1=='Squeezing')):
        amp_imbal_ratio_b2r_squz_vec = np.linspace(1.014,1.02,9)#]

elif(scan_type=='IndAmp'):
    if((seq_type1=='Displacement')or(seq_type1=='Both') or(seq_type1=='Sqz_Clos')):
        # Amp_norm_ind_disp_vec = np.linspace(0.97,1.05,7) #np.linspace(0.6,0.85,15)
        Amp_norm_ind_disp_vec = np.linspace(0.1,1.3,13) #np.linspace(0.6,0.85,15)
    elif(seq_type1=='Squeezing'):
        Amp_norm_ind_squz_vec = np.linspace(0.67,0.57,11)#

elif(scan_type=='phiSK1'):
    # phiSK1_vec = np.linspace(-0.3,0.3,11)
    phiSK1_vec = np.linspace(-0.25,0.25,9)
    # phiSK1_vec = np.linspace(-0.5,0.5,17)

elif(scan_type=='Tdelay'):
    tau_wait_vec = np.linspace(1e1,1.4e4,9)

elif(scan_type=='Phi12'):
    Phi12_vec = Phi12_vec[0]+np.linspace(-0.3,0.3,9)+np.pi/2
    # Phi12_vec = Phi12_vec[0]+np.array([0,0,np.pi,np.pi,0,np.pi,0,np.pi])
elif(scan_type=='Parity'):
    phi_Parity_vec = np.linspace(-0.35,0.35,13)
    # phi_Parity_vec = np.linspace(0,2*np.pi,27)
elif(scan_type=='Nrep'):
    N_gate_rep_vec = np.arange(4)

N_run_freq = 7
N_run_var = 5

w_k_eff_vec_tmp = np.matlib.repmat(np.linspace(-1e2,1e2,N_run_freq),N_run_var,1)
w_k_eff_vec = w_k_eff_vec_tmp.flatten('F')
Amp_norm_ind_disp_tmp = np.matlib.repmat(np.linspace(0.1,0.5,N_run_var),N_run_freq,1)
Amp_norm_ind_disp_vec = Amp_norm_ind_disp_tmp.flatten('C')
print(w_k_eff_vec)
print(Amp_norm_ind_disp_vec)
tau_wait = tau_wait_vec[0]
                                ...
                                for Phi12 in Phi12_vec:
                                    for phi_Parity in phi_Parity_vec:
                                        with qp.build(backend=qp_backend) as out_sched:
                                            ...
                                            ### 2nd sidebands
                                            spin_phase_squz = np.array([0,0,0,0])+np.pi/2
                                            ...
                                            if(is_initial_pi_pulse):
                                                for nn in range(1):
                                                    T_pi_pulse = sequential_sk1(applied_gates=[False,True,False], pi_times=T_pi_vec_microsec,
                                                                    thetas=np.array([np.pi/2,np.pi,np.pi/2]), phis=np.array([0,0, 0])+(np.mod(nn,2)*np.pi)+phiSK1_vec_bare+phiSK1)
                                            ...
                                            def displacement_stage(nn,_motion_phase_offset_disp,_phase_correction1,_phase_correction2_edge,_phase_correction_spin1,_phase_correction_wait1,_phase_correction1_corner):
                                                ...
                                                [duration_disp] = edge(chan_glob_vec=chan_glob_vec,f_carrier_glob = freq_global_vec_SB1, chan_ind=chan_ind_vec[ion_idx_stg],
                                                                            ...
                                                                            phi_motion_offset = phase_motion_disp,n_edge = N_stg_vec[nn],LS = 0) ### displacement of a single ion
                                            ...
                                            def squeezing_stage(nn,_motion_phase_offset_squz,_phase_correction2,_phase_correction1_corner,_phase_correction_spin2,_phase_correction_wait2,_phase_correction2_edge):
                                                ...
                                                [duration_squz] = corner(chan_glob_vec=chan_glob_vec,f_carrier_glob = freq_global_vec_SB2,chan_ind_vec=chan_vec_pad(chan_ind_vec,Ions_squeezing_ind),
                                                                                ...
                                                                                phi_motion_offset = phase_motion_squz,LS = 0)### squeezing of multiple ions
                                                ...
                                                for nn in range(N_stg):
                                                    ...
                                                    if(is_Echo):
                                                        if(np.mod(nn,2)==1):
                                                            ...
                                                            #                 thetas=np.array([np.pi,np.pi,np.pi]), phis=np.array([0.04+np.pi/2, np.pi/2, 0.04+np.pi/2])+(np.pi*phase_echo)+phase_correction_echo+phiSK1_vec_bare)

                                                            # if(nn==1):
                                                            T_corr1 = 0
                                                            T_echo = sequential_sk1(applied_gates=spin_echo, pi_times=T_pi_vec_microsec,
                                                                                thetas=np.array([np.pi,np.pi,np.pi]), phis=np.array([(int(nn==3)+1)*(0.04)+np.pi/2,np.pi/2,((nn==3)+1)*(0.04)+np.pi/2])+(np.pi*phase_echo)+phase_correction_echo+phiSK1_vec_bare)
                                                            # elif(nn==3):
                                                            #     T_corr1 = 0
                                                            #     T_echo = sequential_sk1(applied_gates=spin_echo, pi_times=T_pi_vec_microsec,
                                                            #                         thetas=np.array([np.pi,np.pi,np.pi]), phis=np.array([0.04+np.pi/2, np.pi/2, 0.04+np.pi/2])+(np.pi*phase_echo)+phase_correction_echo+phiSK1_vec_bare)
                                            ...
                                            #                     thetas=np.array([np.pi/2,np.pi/2,np.pi/2]), phis=np.array([0,0,0])+phi_Parity+phase_correction_tot+phiSK1_vec_bare)

                                            if(is_Ramsey):
                                                T_ramsey2 = sequential_sk1(applied_gates=spin_ramsey, pi_times=T_pi_vec_microsec,
                                                                thetas=np.array([np.pi/2,np.pi/2,np.pi/2]), phis=np.array([0.12+0, -0.03+ 0,0.12+ 0])+np.pi+phase_correction_tot+phiSK1_vec_bare+phiSK1)
