...
def corner(chan_glob_vec,f_carrier_glob,chan_ind_vec,f_carrier_ind_vec,chan_ind_pad_vec,freq_ind_pad,
    ...
    ### load splines
    this_dir = pathlib.Path(__file__).parent.resolve()
    json_file = this_dir /'splines_squeeze_compact__mode3_pulsedur029.json' #"splines_squeeze_compact__mode3_pulsedur029.json"
    data_splines = json.loads(json_file.read_text())
...
seq_type1 = 'Squeezing' #### 'Displacement'  |  'Squeezing'  |  'Both'  |  'Sqz_Clos'  |
...
amp_imbal_ratio_b2r_squz_vec =  [1.018]#[1.0182]# [1.0178]# at 0.7 ind amp it is 1.018
...
if(scan_type=='Freq'):
    if((seq_type1=='Displacement')or(seq_type1=='Both') or(seq_type1=='Sqz_Clos')):
        w_k_disp_vec =  (w_k_disp_vec+freq_offset)+np.linspace(-4e3,4e3,31)#np.linspace(2.812e6,2.815e6,17)#np.linspace(2.79e6,2.83e6,15)#   np.linspace(2.8094e6,2.8098e6,15)
    elif((seq_type1=='Squeezing')):
        w_k_squz_vec =  np.linspace(2.18e6,2.8e6,71)#w_k_squz_vec+np.linspace(-3e2,3e2,13)#np.linspace(2.79e6,2.83e6,15)np.linspace(2.808e6,2.813e6,21)#
    ...
    if((seq_type1=='Displacement') or(seq_type1=='Sqz_Clos')):
        ...
        amp_imbal_ratio_b2r_squz_vec = np.linspace(1.015,1.025,11)#]
    ...
    phiSK1_vec = np.linspace(0,2*np.pi,17)
