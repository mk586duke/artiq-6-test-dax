...
def check(number, command):
    if number is not None and number < 0:
        raise FPGAException(
            "OpalKelly exception '{0}' in command {1}".format(
                ErrorMessages.get(number, number), command
            )
        )
...
class OKBase(object):
    ...
    def getDeviceDescription(self, xem):
        """Get information from an open device."""
    ...
    def uploadBitfile(self, bitfile):
        ...
        if self.xem is not None and self.xem.IsOpen():
            check(
                self.xem.ConfigureFPGA(bitfile), "Configure bitfile {0}".format(bitfile)
            )
            self.xem.ActivateTriggerIn(0x41, 9)  # reset overrun
            logger.info(
                "upload bitfile '{0}' to {1}".format(
                    bitfile, self.xem.GetSerialNumber()
                )
            )
    ...
    def openByName(self, name):
        ...
        check(
            self.xem.OpenBySerial(self.modules[name].serial),
