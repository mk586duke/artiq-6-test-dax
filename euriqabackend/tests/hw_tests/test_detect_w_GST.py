...
class Test_GST_Detection(EnvExperiment):
    def build(self):
    ...
    def run(self):
        ...
        print(
            "Detection sequences per run: {}".format(
                self.qubits * (2 + self.C_idx / self.n_loops)
            )
        )

        t_sec_inter = [
            [
                mu_to_seconds(self.t_mu_d1[i][j], self.core) * 10 ** 6
                for j in range(self.n_loops)
            ]
            for i in range(5)
        ]
        t_sec_inter_std = np.std(t_sec_inter, 1)
        t_sec_inter_l = np.sum(t_sec_inter, 1)
        # print([t_sec_inter_l[i]/self.n_loops for i in range(len(t_sec_inter_l))])
        # print(t_sec_inter[4])
        print(t_sec_inter_std)
        for i in range(len(t_sec_inter_l)):
            print(t_sec_inter_l[i] / self.n_loops)
