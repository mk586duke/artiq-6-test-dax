...
class BasicEnvironment(Cooling):
    ...
    def experiment_initialize(self):
        ...
        self.set_experiment_data(
            "config_check_log",
            # [0:3, :, :]: Reorder flag (1: Reordered), Invalid detect flag (1: Invalid detect), Detected qubit number, Detected coolumn number
            np.full((4, self.num_steps, self.num_shots), np.nan),
            broadcast=True,
        )
    ...
    def custom_experiment_initialize(self):
        """Custom additional experiment dataset initialization in the specific experiment."""
        pass
        ...
        while istep < len(self.scan_values):
            ...
            for ishot in range(self.num_shots):
                ...
                self.np_presence_coolant = np.array([2 if np_pmt_count_array_coolant[k] > self.presence_thresh_coolant else 1 for k in
                                       range(self.num_pmts)])
                for i in range(len(self.np_presence_coolant)):
                    self.config[i] = self.np_presence_coolant[i]
                ...
                # Update the config check log cache
                config_check_log[0] = self.reorder_flag
                invalid_detect = 0
                if (self.coolant_countnum != self.n_coolants) or (self.qubit_countnum != self.n_qubits):
                    invalid_detect = 1
                config_check_log[1] = invalid_detect
                config_check_log[2] = self.coolant_countnum
                config_check_log[3] = self.qubit_countnum
                ...
                self.save_data(
                    ...
                    config=self.config,
                    config_check_log=config_check_log
                )
    ...
    def save_data(
        ...
        np_config_check_log = np.array(config_check_log, ndmin=3).T
