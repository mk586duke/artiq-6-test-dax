...
class BasicEnvironment(Cooling):
    ...
    def build(self):
        ...
        self.setattr_argument(
            "presence_thresh",
            artiq_env.NumberValue(default=50, unit="counts", ndecimals=0, scale=1)
        )
        self.setattr_argument(
            "presence_thresh_coolant",
            artiq_env.NumberValue(default=8, unit="counts", ndecimals=0, scale=1)
        )
        ...
        self.setattr_argument(
            "reorder_tolerance",
            artiq_env.NumberValue(default=3, unit="", min=0, ndecimals=0, scale=1)
        )
        self.setattr_argument(
            "keep_global_on",
    ...
    def prepare(self):
        ...
        self.n_qubits = self.get_dataset("global.AWG.N_qubits", archive=False)
        self.n_coolants = self.get_dataset("global.AWG.N_coolants", archive=False)
        ...
        # Sorting
        # counted chain configuration
        self.config = np.array([0] * self.num_pmts)
        self.np_presence_coolant = np.array([0] * self.num_pmts)
        self.np_presence_qubit = np.array([0] * self.num_pmts)
        self.reorder_flag = 0
        # counted numbers
        self.qubit_countnum = 0
        self.coolant_countnum = 0
    ...
    def experiment_initialize(self):
        ...
        # LE 7/19/2019 (moved ahead of set switch network so setRunningSN(False)
        # can be set to avoid programming failure)
        # self.kn_initialize()

        # RFSoC is connected to port 2
        # TODO: remove once we don't need to switch back to AWG
        awg_rfsoc_switch = rf_switch.SPDTNetworkSwitch("192.168.80.12")
    ...
    def experiment_loop(self):
        ...
        ss_counts_rev = [0] * self.num_pmts
        ss_coolant_counts_rev = [0] * self.num_pmts
        ...
        while istep < len(self.scan_values):
            ...
            for ishot in range(self.num_shots):
                ...
                delay(25 * us)
                cool_attempts = 0
                # max_cool_attempts must be no smaller than 1
                max_cool_attempts = 1
                passed = False
                ...
                while (not passed) and (cool_attempts < max_cool_attempts):
                    ...
                    delay(30 * us)  # To give some slack back after reading PMTs
                    ...
                    if self.do_SS_cool:
                        ...
                        stopcounter_ss_edge_mu = now_mu()

                        stopcounter_ss_edge_coolant_mu = now_mu()

                        delay(20 * us)


                    # correct edge ion counts and collect total ss counts
                    total_ss_counts = 0
                    total_ss_coolant_counts = 0

                    if self.do_SS_cool:
                        if self.num_pmts >= 4:
                            for i in range(self.num_pmts):
                                # Revise here to check the edge/middle ion counts
                                if (i==0) or (i==1) or (i==self.num_pmts - 2) or (i==self.num_pmts - 1):
                                    ss_counts_rev[i] = ss_edge_counts[i]
                                    ss_coolant_counts_rev[i] = ss_edge_coolant_counts[i]
                                    total_ss_counts += ss_edge_counts[i]
                                    total_ss_coolant_counts += ss_edge_coolant_counts[i]
                                else:
                                    ss_counts_rev[i] = ss_counts[i]
                                    ss_coolant_counts_rev[i] = ss_coolant_counts[i]
                                    total_ss_counts += ss_counts[i]
                                    total_ss_coolant_counts += ss_coolant_counts[i]
                        else:
                            for i in range(self.num_pmts):
                                ss_counts_rev = ss_counts
                                ss_coolant_counts_rev = ss_coolant_counts
                                total_ss_counts += ss_counts[i]
                                total_ss_coolant_counts += ss_coolant_counts[i]

                    if self.do_SS_cool and self.lost_ion_monitor:
                        if (total_ss_counts < 1 * self.n_qubits) or (total_ss_coolant_counts < 1 * self.n_coolants):
                ...
                # Avoid RTIO underflow when max_cool_attempts = 0
                delay(2 * ms)
                ...
                if self.reorder_flag == 0:
                    prev_config = self.config

                # Chain configuration check with only 172 counts
                # Check with 171 counts is not working well due to insufficient 14.7 GHz sideband
                # np_pmt_count_array_coolant = np.array(ss_coolant_counts_rev)
                np_pmt_count_array_coolant = np.array(ss_edge_coolant_counts)
                self.np_presence_coolant = np.array([2 if np_pmt_count_array_coolant[k] > self.presence_thresh_coolant else 1 for k in
                                       range(self.num_pmts)])
                self.config = self.np_presence_coolant

                # Reorder detection
                # set tolerance for reorder to be 3: if detected config is checked to be different consecutively for 3 times from the
                # latest good config, decide that the chain is reordered, and use the latest check counts (over the 3
                # additional detection) to get current config.

                reorder_check = self.reorder_detect(prev_config, self.config, ishot, istep)
                if reorder_check == 0:
                    self.reorder_flag = 0
                else:
                    self.reorder_flag += reorder_check
                if self.reorder_flag == self.reorder_tolerance:
                    # TODO: put sorting functions here
                    # self.reorder_flag = 0
                    pass

                # Update counted qubit and coolant number
                for i in range(len(self.config)):
                    if self.config[i] == 2:
                        self.coolant_countnum += 1
                    elif self.config[i] == 1:
                        self.qubit_countnum += 1
                ...
                self.save_reorder(self.reorder_flag, ishot, istep)

                self.save_config(self.config, ishot, istep)
    ...
    def save_counts(
        ...
        ss_edge_counts: TList(TInt32),
        ss_edge_coolant_counts: TList(TInt32),
        ...
        if ishot == 0:
            ...
            self.ss_edge_monitor = np.full(len(ss_edge_counts), 0)
            self.ss_edge_coolant_monitor = np.full(len(ss_edge_coolant_counts), 0)
        ...
        self.ss_edge_monitor += np.array(ss_edge_counts)
        self.ss_edge_coolant_monitor += np.array(ss_edge_coolant_counts)
        ...
        if ishot == self.num_shots - 1:
            ...
            self.ss_edge_monitor = np.array(self.ss_edge_monitor / self.num_shots, ndmin=2).T
            self.mutate_dataset(
                "data.monitor.sscooling_edge",
                ((0, self.num_pmts, 1), (istep, None, self.num_steps)),
                self.ss_edge_monitor,
            )
            self.ss_edge_coolant_monitor = np.array(self.ss_edge_coolant_monitor / self.num_shots, ndmin=2).T
            self.mutate_dataset(
                "data.monitor.sscooling_edge_coolant",
                ((0, self.num_pmts, 1), (istep, None, self.num_steps)),
                self.ss_edge_coolant_monitor,
            )
            ...
            #         _LOGGER.error("LOST IONS, %f", istep)
            #         self.set_dataset("monitor.Lost_Ions", True, broadcast=True)
        ...
        np_ss_edge_counts = np.array(ss_edge_counts, ndmin=3).T
        np_ss_edge_coolant_counts = np.array(ss_edge_coolant_counts, ndmin=3).T
    ...
    def save_sort_sol(
        self,
        sortlog: TList(TInt32),
        ishot: TInt32,
        istep: TInt32,
    ) -> TNone:
        # Saving the target configuration after sorting (for checking sorting)

        np_sortlog = np.array(sortlog, ndmin=3).T
    ...
    def kn_idle(self):
        ...
        # SHOULDN'T PUT RF LOCKCONTROL HERE, EXECUTED BEFORE CUSTOM IDLE, LOCK ON BEFORE RAMP UP
    ...
    @kernel
    def sorting_sol_generator(self, DC, config, sorting_sol) -> TList(TInt32):
        # 172: 2; 171: 1
        # numpy has nonetype problems unique to kernel
        DC_coolant_index = []
        CC_coolant_index = []
        # Create local variable to avoid overwriting config
        CC = []
        ...
        if len(CC_coolant_index) == len(DC_coolant_index):
