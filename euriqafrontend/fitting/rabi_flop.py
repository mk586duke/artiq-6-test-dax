...
"""THIS FILE ADAPTED FROM EXFORD OITG PACKAGE, modified so Rabi starts at 0
...
def fitting_function(x, p):
    ...
    y = p["y_upper"]/2 - (p["y_upper"] - y_lower) / 2 * (np.exp(-shifted_t / p["tau_decay"]) * np.cos(2 * np.pi / p["t_period"] * shifted_t))
    return np.where(x < p["t_dead"], y_lower, y)
...
rabi_flop = FitBase.FitBase(
    ["t_period", "t_dead", "y_upper", "tau_decay"],
    fitting_function, parameter_initialiser=parameter_initialiser,
    derived_parameter_function=derived_parameter_function,
    parameter_bounds={"t_period": (0, np.inf), "t_dead": (0, np.inf), "y_upper": (0, 1), "tau_decay": (0, np.inf)})
