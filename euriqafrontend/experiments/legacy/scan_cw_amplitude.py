...
class TestOutputs(EnvExperiment):
    """Scan CW Amplitude (single channel)"""
    ...
    def build(self):
        ...
        self.setattr_argument(
            "frequency_dds", NumberValue(default=50e6, unit="MHz", ndecimals=3, min=0)
        )
        self.setattr_argument(
            "amplitude_scan",
            Scannable(
                default=LinearScan(1023, 0, 1024),
                unit="",
                global_step=1,
                global_min=0,
                global_max=1023,
    ...
    def prepare(self):
        ...
        self.qubit_ampl = [np.int(i) for i in self.amplitude_scan]
        self.frequency_dds_bin = self.freq_to_bin(self.frequency_dds)
    ...
    def run_exp(self):
        ...
        self.dds_freq(self.qubit_dds, self.frequency_dds_bin)
        self.dds_ampl(self.qubit_dds, self.qubit_ampl[0])
        delay(1 * s)
        for i in self.qubit_ampl:
            self.dds_ampl(self.qubit_dds, i)
            delay(self.amplitude_time_s)
    ...
    def exp_init(self):
        ...
        self.dds_freq(self.qubit_dds, self.frequency_dds_bin)  # Zero 'time'
        delay(10 * us)  # Need to give some room for bus to clear
        self.dds_ampl(self.qubit_dds, self.qubit_ampl[0])  # Zero 'time'
        pass
