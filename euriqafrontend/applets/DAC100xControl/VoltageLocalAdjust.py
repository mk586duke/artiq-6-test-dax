...
from .data_models import VoltageLocalAdjustTableModel
...
uipath = os.path.join(os.path.dirname(__file__), "VoltageLocalAdjust.ui")
...
class LocalAdjustRecord(object):
    def __init__(self, name="", path=None, gain=0, globalDict=None):
        ...
        self.gain = (
            gain
            if isinstance(gain, ExpressionValue)
            else ExpressionValue(name, globalDict, gain)
        )
    ...
    def solution(self, sol):
        ...
        if self._solution:
            hash(
                tuple(
                    (
                        hashlib.sha256(a.view(numpy.uint8)).hexdigest()
                        for a in self._solution
                    )
                )
            )
        else:
...
class VoltageLocalAdjust(Form, Base):
        ...
        for record in self.localAdjustList:
            ...
            try:
                record.gain.valueChanged.connect(
                    partial(self.onValueChanged, record), QtCore.Qt.UniqueConnection
                )
            except:
                pass
    ...
    def setupUi(self, parent):
        ...
        self.tableModel = VoltageLocalAdjustTableModel(
            self.localAdjustList, self.channelDict, self.globalDict
        )
    ...
    def onAdd(self):
        ...
        try:
            newrecord.gain.valueChanged.connect(
                partial(self.onValueChanged, newrecord), QtCore.Qt.UniqueConnection
            )
        except:
            pass
    ...
    def onRemove(self):
        for index in sorted(
            unique([i.row() for i in self.tableView.selectedIndexes()]), reverse=True
        ):
    ...
    def outputChannels(self):
        self._outputChannels = dict(
            (
                (
                    record.name,
                    VoltageOutputChannel(self, None, record.name, self.globalDict),
                )
                for record in self.localAdjustList
            )
        )
