...
from .MagnitudeParser import isValueExpression
...
class MagnitudeSpinBoxDelegateMixin(object):
    def createEditor(self, parent, option, index):
        ...
        editor = MagnitudeSpinBox(
            parent,
            globalDict=localDict,
            valueChangedOnEditingFinished=False,
            emptyStringValue=self.emptyStringValue,
        )
    ...
    def setEditorData(self, editor, index):
        ...
        try:
            numberlen = len(
                re.split(r"([+-]?[0-9\.]+(?:[eE][0-9]+)?)(.*)", str(value))[1]
            )
    ...
    def setModelData(self, editor, model, index):
        ...
        model.setData(
            index, text if not isValueExpression(text) else None, QtCore.Qt.UserRole
        )  # is parsable thus must be a magnitude without math
        model.setData(index, value, QtCore.Qt.EditRole)
        # DisplayRole would be better, for backwards compatibility we leave it
        # at EditRole and distinguish there by type


class MagnitudeSpinBoxDelegate(
    QtWidgets.QStyledItemDelegate, MagnitudeSpinBoxDelegateMixin
):
