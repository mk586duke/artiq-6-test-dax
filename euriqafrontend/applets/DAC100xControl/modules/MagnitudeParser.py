...
# *****************************************************************
"""
Parser for magnitude expressions.

Can parse arithmetic expressions with values including standard SI units.
"""
import logging
...
hexnum = (
    (Literal("0x") + Word(srange("[0-9a-fA-F]")))
    .setResultsName("hex")
    .setParseAction(hexConversion)
)
integer = (
    Word("+-" + nums, nums).setResultsName("integer").setParseAction(integerConversion)
)
dotNumber = Combine(
    Optional(plus | minus) + point + Word(nums) + Optional(e + Word("+-" + nums, nums))
).setParseAction(floatConversion)
numfnumber = Combine(
    Optional(plus | minus)
    + Word(nums)
    + point
...
def parse(string):
    try:
        ...
        retval = Q(
            float(val[0]), val[1] if len(val) > 1 else None
        )  # TODO: might need precision
    except ParseException as e:
        _LOGGER.error("Error parsing '%s' using MagnitudeParser", string)
        raise
    return retval
