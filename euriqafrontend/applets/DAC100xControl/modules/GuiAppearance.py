...
from .TableViewExtended import TableViewExtended
...
def restoreColumnWidth(tableView, widthData, autoscaleOnNone=True):
    if widthData and len(widthData) == tableView.model().columnCount():
        for column, width in zip(
            list(range(0, tableView.model().columnCount())), widthData
        ):
...
appearanceHelpers = {
    QtWidgets.QSplitter: (
        QtWidgets.QSplitter.saveState,
        QtWidgets.QSplitter.restoreState,
    ),
