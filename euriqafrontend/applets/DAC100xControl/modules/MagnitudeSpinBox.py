...
# *****************************************************************
"""
A QSpinBox for Physical quantities.

It accepts for example "10 MHz".
Features are: up-down arrows will increase/decrease the digit
...
class MagnitudeSpinBox(QtWidgets.QAbstractSpinBox):
    ...
    def __init__(
        ...
        self.orangeTextPalette.setColor(
            QtGui.QPalette.Text, QtGui.QColor(0x7d, 0x05, 0x52)
        )
    ...
    def validate(self, inputstring, pos):
        try:
            ...
            if (
                ...
                and value.dimensionality != self._dimension.dimensionality
            ):
    ...
    def stepBy(self, steps):
        try:
            ...
            value, delta, pos, decimalpos, prec = MagnitudeParser.parseDelta(
                lineEdit.text(), lineEdit.cursorPosition()
            )
            newvalue = value + (steps * delta)
            newtext = format(newvalue, "~.{}f".format(prec))
            self.lineEdit().setText(newtext)
            value, delta, _, newdecimalpos, prec = MagnitudeParser.parseDelta(
                newtext, lineEdit.cursorPosition()
            )
    ...
    def interpretText(self):
        _LOGGER.debug("interpret text")

    def fixup(self, inputstring):
        _LOGGER.debug("fixup '%s'", inputstring)
            ...
            if len(text) > 0:
                ...
                if (
                    ...
                    raise DimensionMismatch(
                        "Got unit '{0}' expected '{1}'".format(
                            value.dimensionality, self._dimension.dimensionality
                        )
                    )
            else:
