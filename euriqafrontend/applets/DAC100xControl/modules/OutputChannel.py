...
from .ExpressionValue import ExpressionValue
from .InstrumentSettings import InstrumentSettings
from .quantity import is_Q
from .quantity import Q
from .useTracker import UseTracker
...
def nextValue(current, target, stepsize, jump):
    if (current is None) or jump:
        ...
        return (
            (target, True)
            if abs(temp) <= stepsize
            else (current + numpy.sign(temp) * stepsize, False)
        )
...
class OutputChannel(QtCore.QObject):
    ...
    def __init__(
        self, device, deviceName, channelName, globalDict, settings=None, outputUnit=""
    ):
    ...
    def setDefaults(self):
        self.settings.__dict__.setdefault(
            "value", Q(0, self.outputUnit)
        )  # the current value
        self.settings.__dict__.setdefault(
            "persistDelay", Q(60, "s")
        )  # delay for persistency
        self.settings.__dict__.setdefault(
            "strValue", None
        )  # requested value as string (formula)
        self.settings.__dict__.setdefault(
            "targetValue", Q(0, self.outputUnit)
        )  # requested value as string (formula)
    ...
    def name(self):
        return (
            "{0}_{1}".format(self.deviceName, self.channelName)
            if self.channelName
            else self.deviceName
        )
    ...
    def setValue(self, targetValue):
        """
        go stepsize towards the value.

        This function returns True if the value is reached.
        Otherwise it should return False.
        The user should call repeatedly until the intended value is reached
    ...
    def persist(self, channel, value):
        ...
        self.decimation.decimate(
            time.time(), value, partial(self.persistCallback, decimationName)
        )
    ...
    def restoreValue(self):
        """
        restore the value saved previously.

        This routine only goes stepsize towards this value
        if the stored value is reached returns True, otherwise False.
        Needs to be called repeatedly until it returns True in order
    ...
    @property
    def parameter(self):
        """Prevent unconnected signal issue by re-creating the parameters each time."""
        self._parameter = Parameter.create(
            name=self.name, type="group", children=self.paramDef()
        )
        self._parameter.sigTreeStateChanged.connect(
            self.update, QtCore.Qt.UniqueConnection
        )
    ...
    def update(self, param, changes):
        ...
        called by the signal of pyqtgraph parametertree
        """
...
class SlowAdjustOutputChannel(OutputChannel):
    def __init__(
        self, device, deviceName, channelName, globalDict, settings, outputUnit
    ):
        super(SlowAdjustOutputChannel, self).__init__(
            device, deviceName, channelName, globalDict, settings, outputUnit
        )
    ...
    def setDefaults(self):
        ...
        self.settings.__dict__.setdefault(
            "delay", Q(100, "ms")
        )  # s delay between subsequent updates
        self.settings.__dict__.setdefault(
            "jump", False
        )  # if True go to the target value in one jump
        self.settings.__dict__.setdefault(
            "stepsize", Q(1, self.outputUnit)
        )  # if True go to the target value in one jump
    ...
    def setValue(self, targetValue):
        ...
        it should return False. The user should call repeatedly until
        the intended value is reached and True is returned.
        """
        if targetValue is not None:
            self.settings.targetValue = targetValue
            newvalue, arrived = nextValue(
                self.settings.value,
                targetValue,
                self.settings.stepsize,
                self.settings.jump,
            )
            reportvalue, arrived = self.encpasulate_value(
                self.device.setValue(self.channelName, newvalue), second=arrived
            )
            self.settings.value = reportvalue
            self.expressionValue.value = (
                targetValue
            )  # TODO: still unclear whether that makes sense
            self.valueChanged.emit(self.settings.value)
            if arrived:
    ...
    def expressionUpdateBottomHalf(self):
        if not self.setValue(self.settings.targetValue):
            QtCore.QTimer.singleShot(
                self.settings.delay.m_as("ms"), self.expressionUpdateBottomHalf
            )
        else:
            self.useTracker.release(self.name)
            self.timerActive = False
