...
# *****************************************************************
"""
Parses arithmetic, units and custom functions in combo boxes.
...
from .ExprFuncDecorator import ExpressionFunctions
...
class Parser:
    def __init__(self, variabledict=None, functiondict=None):
        ...
        self.defaultFuncCM = ChainMap(
            ExpressionFunctions, self.localFunctions, functiondict
        )
    ...
    def p_error(self, p):
        raise ExpressionError(
            "Syntax error at '{0}' in '{1}'".format(p.value, p.lexer.lexdata)
        )
...
class Expression:
    ...
    def evaluate(
        ...
        if functiondict is None:
            functiondict = dict()
        return self.exprParser.evaluate(
            s, variabledict, listDependencies, useFloat, functiondict
        )
    ...
    def evaluateAsMagnitude(
        ...
        if functiondict is None:
            variabledict = dict()
        return self.exprParser.evaluateAsMagnitude(
            s, variabledict, listDependencies, useFloat, functiondict
        )
...
if __name__ == "__main__":
    ...
    def test(s, expVal, variabledict=None, listDependencies=False):
        if variabledict is None:
            variabledict = dict()
        try:
            if listDependencies:
                vall, deps = ExprEval.evaluate(
                    s, variabledict, listDependencies, useFloat=True
                )
            else:
                vall = ExprEval.evaluate(
                    s, variabledict, listDependencies, useFloat=False
                )
            if vall == expVal:
                print(s, "=", expVal)
            else:
    ...
    test(
        "kwfun(tempfun(4),2,3,f=10,d=9)",
        UserFunctions.kwfun(UserFunctions.tempfun(4), 2, 3, f=10, d=9),
    )
    test(
        "kwfun(tempfun(4),2,3,f=10,d=myvar,verbose=1)",
        UserFunctions.kwfun(UserFunctions.tempfun(4), 2, 3, f=10, d=124),
        {"myvar": 124},
