...
from . import ExprFuncDecorator as trc  # NamedTraceDict
...
class ExpressionValue(QtCore.QObject):
    ...
    def __eq__(self, other):
        return (
            other is not None
            and isinstance(other, ExpressionValue)
            and (self.name, self._string, self._value)
            == (other.name, other._string, other._value)
        )
    ...
    def string(self, s):
        if self._globalDict is None:
            raise ExpressionValueException(
                "Global dictionary is not set in {0}".format(self.name)
            )
        self._string = s
        ...
        self.registrations[:] = []
        if self._string:
            val, dependencies = self.expression.evaluateAsMagnitude(
                self._string, self._globalDict, listDependencies=True
            )
            if callable(val.m):
                self.func = deepcopy(val.m)
                if any("NamedTraceDict" in key for key in val.m.__code__.co_names):
                    dependencies.add("__namedtrace__")
                    # hold off if function depends on NamedTraceDict
                    # and NamedTraces haven't been loaded
    ...
    def recalculate(self, name=None, value=None, origin=None, forceUpdate=False):
        ...
        if self._string:
            newValue = self.expression.evaluateAsMagnitude(
                self._string, self._globalDict
            )
            if callable(newValue.m):
                if not (
                    not trc.NamedTraceDict
                    and "__namedtrace__" in (dep[0] for dep in self.registrations)
                ):
                    self.func = deepcopy(newValue.m)
                    self._value = Q(newValue.m())
                self._updateFloatValue()
                self.valueChanged.emit(
                    self.name, self._value, self._string, "recalculate"
                )
            else:
                self.func = self._returnVal
